        ...
        <div className="flex items-center justify-between h-[68px]">
          <Link to="/" className="flex items-center gap-3 group shrink-0">
            <div className="relative">
              <div className="w-10 h-10 rounded-xl gradient-primary flex items-center justify-center text-white font-bold text-xl shadow-glow group-hover:scale-105 transition-transform duration-300">
                C
              </div>
              <span className="absolute -right-1 -bottom-1 w-3.5 h-3.5 rounded-full bg-emerald-400 ring-2 ring-white dark:ring-slate-900 shadow-sm" />
            </div>
            <div className="hidden sm:block">
              <div className="text-[17px] font-extrabold tracking-tight text-slate-900 dark:text-white font-heading">Campus<span className="text-gradient">Notes</span></div>
              <div className="text-[10px] font-bold text-slate-400 tracking-[0.15em]">STUDY • SHARE • GROW</div>
            </div>
          </Link>

          <div className="hidden md:flex items-center gap-2">
            <Link to="/" className="px-4 py-2 rounded-xl text-sm font-bold text-slate-600 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-300 dark:hover:text-white dark:hover:bg-slate-800 transition-all">Home</Link>
            <Link to="/notes" className="px-4 py-2 rounded-xl text-sm font-bold text-slate-600 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-300 dark:hover:text-white dark:hover:bg-slate-800 transition-all">Browse Notes</Link>

            {isAuthenticated && (
              <Link to="/requests" className="px-4 py-2 rounded-xl text-sm font-bold text-slate-600 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-300 dark:hover:text-white dark:hover:bg-slate-800 transition-all">Requests</Link>
            )}
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-2 ml-3 pl-3 border-l border-slate-200 dark:border-slate-800">
                ...
                <Link to="/upload" className="btn-gradient py-2 px-5 text-sm ml-2">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>
                  Share Notes
                </Link>

                <div className="relative group ml-2">
                  <button className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 font-bold text-sm flex items-center justify-center ring-2 ring-transparent hover:ring-brand-light transition-all shadow-sm">
                    {getInitials(user?.name)}
                  </button>
                  <div className="absolute right-0 mt-3 w-64 bg-white dark:bg-slate-900 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-800 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform origin-top-right group-hover:scale-100 scale-95 overflow-hidden">
                    <div className="px-5 py-4 bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 backdrop-blur-sm">
                      <p className="text-sm font-extrabold text-slate-900 dark:text-white">{user?.name}</p>
                      <p className="text-xs text-slate-500 font-medium truncate mt-1">{user?.email}</p>
                    </div>
                    <div className="p-2 space-y-1">
                      <Link to="/profile" className="block px-4 py-2.5 rounded-2xl text-sm font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">My Profile</Link>
                      {isAdmin && <Link to="/admin" className="block px-4 py-2.5 rounded-2xl text-sm font-bold text-brand hover:bg-brand-light/10 transition-colors">Admin Analytics</Link>}
                      <button onClick={handleLogout} className="w-full text-left px-4 py-2.5 rounded-2xl text-sm font-bold text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-colors">Log out</button>
                    </div>
                  </div>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-3 ml-3 pl-3 border-l border-slate-200 dark:border-slate-800">
